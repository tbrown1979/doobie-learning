...
trait CoffeeRepositoryComponent {
  val coffeeRepository: CoffeeRepository

  trait CoffeeRepository {
    def get(supId: Int): Task[Option[Coffee]]
    //def update(coffee: Coffee): Task[Option[Coffee]]
  }
}

trait DoobieCoffeeRepositoryComponent extends CoffeeRepositoryComponent with DoobieProfile {
  val coffeeRepository = new DoobieCoffeeRepository{}

  trait DoobieCoffeeRepository extends CoffeeRepository with CoffeeConstructors {
    def get(supId: Int): Task[Option[Coffee]] =
      allCoffees.filter(c => c.supId == supId).take(1).list.transact(db).map(_.headOption)
  }
}
